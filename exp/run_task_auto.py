...
with open("config_auto.json") as f:
    param = json.load(f)

participants = [colink.Participant(user_id=users["test-0"]["user_id"], role="server")]
for i in range(1,179):
    participants.append(colink.Participant(user_id=users["test-{}".format(i)]["user_id"], role="client"))
...
param["deployment"]["mode"] = "colink"
task_id = cls["test-0"].run_task("unifed.flower", json.dumps(param), participants, True)
